package ru.deathcry.bigbobby.c_theory.question14.phones
...
class PhonesController {
    ...
    //todo K modify correct method to order/sort chairs -  What chairs :D?
    ...
    @GetMapping
    fun listAllPhones(
            @RequestParam manufacturer: String? = null,
            @RequestParam minPrice: Int? = null,
            @RequestParam maxPrice: Int? = null,
            @RequestParam sortByColumn: String = "mostBuys", // use `?sortByColumn=releaseDate` to sort by release
            @RequestParam order: String = "desc" // use `?order=asc` to sort in ascending order
    ): List<Phone> {
    ...
    @GetMapping("/{model}")
    fun findPhoneByName(@PathVariable model: String): Phone? {
        return null
    }

    //todo D create a method to save a phone
    @PostMapping
    fun savePhone(@RequestBody phone: Phone): String {
        return "OK: Phone saved"
    }

    //todo E create a method to update a phone
    @PutMapping("/{name}")
    fun updatePhone(@PathVariable name: String, @RequestBody phone: Phone): String {
        return "OK: Phone updated"
    }

    //todo F create a method to delete a phone
    @DeleteMapping("/{name}")
    fun deletePhone(@PathVariable name: String): String {
        return "OK: Phone deleted"
    }
    ...
    fun getAppsOnPhone(@PathVariable name: String): List<App> {
        return listOf()
    }
    ...
    fun setNewPhonePrice(@PathVariable name: String, @RequestBody priceDto: PriceDto): List<App> {
        return listOf()
    }
}
